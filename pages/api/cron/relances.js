...
export default async function handler(req, res) {
  ...
  if (erreurEntreprises) {
    return res.status(500).json({ error: erreurEntreprises.message });
  }

  for (const entreprise of entreprises) {
    const nomEnt = nomEntreprise(entreprise);

    // 1) Devis sans réponse
    const { data: devisAReleancer } = await supabaseAdmin
      .from('documents')
    ...
    for (const devis of devisAReleancer || []) {
      ...
      const html = `
        ...
        (${formatMontant(devis.total_ttc)} TTC), envoyé le ${formatDate(new Date(devis.envoye_le))}, resté sans réponse à ce jour.</p>
        <p>N'hésitez pas à me faire part de votre décision ou de vos éventuelles questions.</p>
    ...
    // 2) Factures impayées : échéance proche ou dépassée
    const delaiPaiement = entreprise.delai_paiement_jours || 30;
    const { data: facturesEnCours } = await supabaseAdmin
      .from('documents')
    ...
    for (const facture of facturesEnCours || []) {
      ...
      const sujet = enRetard
        ...
        : `Rappel — Facture ${facture.numero} à régler bientôt — ${nomEnt}`;
      const html = enRetard
        ? `
          <p>Bonjour ${facture.client_nom || ''},</p>
          <p>Sauf erreur de notre part, la facture <strong>${facture.numero}</strong>
          (${formatMontant(facture.total_ttc)} TTC), dont l'échéance était fixée au ${formatDate(dateEcheance)},
          reste à ce jour impayée.</p>
          <p>Merci de bien vouloir procéder au règlement dans les meilleurs délais, ou de nous contacter en cas de difficulté.</p>
          <p>Cordialement,<br>${nomEnt}</p>
        `
        : `
          <p>Bonjour ${facture.client_nom || ''},</p>
          <p>Petit rappel amical : la facture <strong>${facture.numero}</strong>
          (${formatMontant(facture.total_ttc)} TTC) arrive à échéance le ${formatDate(dateEcheance)}.</p>
